...
describe("get_smarttype_times_of_day", () => {
  test("path is required", async () => {
    expect((await handleGetSmarttypeTimesOfDay({})).isError).toBe(true);
  });

  test("returns entries and the effective separator on a leading line", async () => {
    await handleReadFdx({ path: FIXTURE_PATH });
    const result = await handleGetSmarttypeTimesOfDay({ path: FIXTURE_PATH });
